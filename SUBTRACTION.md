...
2. **Held-out verification.** Check the candidate on later audio that doesn't overlap the calibration block, in 50 ms sub-blocks, separately per channel, at unity gain:
   ...
   - residual projections `⟨C−R, R shifted by k⟩/⟨R,R⟩` for `k = ±1…±16`. On a sample-identical endpoint `C−R` is only the other apps, so each reads ≈ 0. An endpoint effect (EQ, spatialiser, a device enhancement APO) can keep gain at 1 and delay at 0 while a filtered copy of own audio survives the subtraction; it shows up here. Example: taps of 0.15 at ±4 frames leave ≈ −13 dB of own audio yet read gain 1.000, delay 0.

   Accept only when, for every channel with at least 8 valid sub-blocks, `|mean − target| + 2·se` lies inside the tolerance: gain within 1 ± 0.03, delay within 0 ± 0.05 samples, every residual projection within 0 ± 0.03. It's an equivalence-style test. Other apps are independent of the reference, so they only add noise, and noise can only delay acceptance, never cause it. A conclusive rejection drops the candidate. So does being still inconclusive after 200 sub-blocks (10 s).
3. **Running monitor.** The same estimates keep running over non-overlapping 20-sub-block windows. Two consecutive windows that reject at `z = 4` fault the session with "lost lock". This catches clock slips, lock loss the timing didn't flag, own audio that stops reaching the endpoint, and an endpoint effect that switches on mid-share.
